...
class FoTOCRenderer {
    ...
    public FoTOCRenderer(Headers headers) {
     ...
     */
    public final void renderBookmark(@NonNull Document doc, FoWriter writer) {
    ...
    private void renderBookmark(@NonNull Header node, @NonNull FoNode tree,
        ...
        bookmark.setTitle(headers.getChapterTitle(node));
     ...
     */
    public final void renderToc(Document doc, FoWriter writer) {
        ...
        doc.elements().stream()
            ...
            .forEach(header -> {
                ...
                inline.setKeepWithNext("always")
                    .addText(Fo.encode(headers.getChapterTitle(header)));
    ...
    private static String asRomanNumber(int number) {
        ...
        int i = 0;
        while ((number > 0) || (FoTOCRenderer.ARAB.length == (i - 1))) {
            while ((number - FoTOCRenderer.ARAB[i]) >= 0) {
                number -= FoTOCRenderer.ARAB[i];
                result.append(FoTOCRenderer.ROMAN[i]);
            }
            i++;
        }
     ...
     */
    public static Headers createHeaders(Document doc) {
            ...
            if (header.getLevel() == 1)
                ...
                for (var i = 1; i < stack.size(); i++) {
                    ...
                    if (level == 0) {
                        ...
                        value += (isRoman ? FoTOCRenderer.asRomanNumber(level) : level) + ".";
                }
