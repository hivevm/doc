...
import org.hivevm.railroad.svg.SvgLayout.LineContinuationType;

public class SvgDiagram {

    private final Railroad rrElement;

    public SvgDiagram(Railroad rrElement) {
    ...
    public static class SvgContent {
        private final List<SvgConnector> connectorList = new ArrayList<>();

        public void addPathConnector(int x1, int y1, String path, int x2, int y2) {
            Object c = connectorList.isEmpty() ? null : connectorList.getLast();
            if (c != null) {
        ...
        public void addLineConnector(int x1, int y1, int x2, int y2) {
            ...
            Object c = connectorList.isEmpty() ? null : connectorList.getLast();
            if (!(c instanceof SvgLine) || !((SvgLine) c).mergeLine(x1_, y1_, x2_, y2_)) {
                connectorList.add(new SvgLine(x1_, y1_, x2_, y2_));
            }
        }

        private String getConnectorElement(SvgLayout rrDiagramToSVG) {
            ...
            String connectorColor = Svg.toString(rrDiagramToSVG.getConnectorColor());
        ...
        private final StringBuilder elementsSB = new StringBuilder();
        ...
        private final Map<String, String> cssClassToDefinitionMap = new HashMap<>();
        ...
        private String getCSSStyles() {
            ...
            for (String aClass : cssClasses) {
                ...
                String definition = cssClassToDefinitionMap.get(aClass);
                if (definition.endsWith(";")) {
                    sb.append(".").append(aClass).append("{").append(definition).append("}");
                }
            }
            return sb.toString();
        }
    ...
    public final String toSVG(SvgLayout context) {
        List<Railroad> rrElementList = new ArrayList<>();
        if (rrElement instanceof Railroad.Sequence) {
            List<Railroad> cursorElementList = new ArrayList<>();
            for (Railroad element : ((Railroad.Sequence) rrElement).rrElements()) {
                if (element instanceof Railroad.Break) {
                    if (!cursorElementList.isEmpty()) {
                        rrElementList.add(cursorElementList.size() == 1 ? cursorElementList.getFirst() : new Railroad.Sequence(cursorElementList));
        ...
        Railroad startElement = context.getStartElement();
        Railroad endElement = context.getEndElement();
        if (startElement != null || endElement != null) {
            if (startElement != null) {
                List<Railroad> list = Arrays.asList(startElement, rrElementList.getFirst());
                rrElementList.set(0, new Railroad.Sequence(list));
            }
            if (endElement != null) {
                List<Railroad> list = Arrays.asList(rrElementList.get(rrElementListCount - 1), endElement);
                rrElementList.set(rrElementListCount - 1, new Railroad.Sequence(list));
            }
        }
        LineContinuationType lineContinuationType = context.getLineContinuationType();
        ...
        for (int i = 0; i < rrElementListCount; i++) {
            ...
            Railroad rrElement = rrElementList.get(i);
            LayoutData layoutInfo = LayoutData.create(rrElement, context);
            int computedWidth = 5 + layoutInfo.width() + 5;
        ...
        boolean isLeftAligned = context.isLeftAligned();
        ...
        for (int i = 0; i < rrElementListCount; i++) {
            ...
            boolean isFirst = i == 0;
            boolean isLast = i == rrElementListCount - 1;
            LayoutData layoutInfo2 = context.getLayoutData(rrElement);
            ...
            SvgRenderer.render(rrElement, x, yOffset, context, svgContent);
            x += width2;
        ...
        String connectorElement = svgContent.getConnectorElement(context);
    ...
    public static SvgDiagram toDiagram(Railroad elem) {
        return new SvgDiagram(elem);
    }
}
