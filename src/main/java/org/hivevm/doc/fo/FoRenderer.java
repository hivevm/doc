    ...
    protected final void addAndReplaceText(String text, FoBlock block) {
        ...
        while (matcher.find()) {
            if (offset < matcher.start()) {
                block.addNode(FoBlock.inline(block.getBuilder()).addContent(text.substring(offset, matcher.start())));
            }

            FoBasicLink link = new FoBasicLink("mailto:" + matcher.group(2), block.getBuilder());
