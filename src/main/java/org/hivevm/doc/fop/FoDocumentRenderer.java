...
class FoDocumentRenderer implements DocumentVisitor<FoContext> {
    ...
    public final void visit(Document doc, FoContext data) {
        ...
        data.createFlow(FoDocumentRenderer.BOOK_ID, Fo.PAGESET_BOOK, false, properties);
        ...
        // Table of Content
        data.createFlow("id_toc", Fo.PAGESET_STANDARD, true, properties);
        createToc(doc, data);
    }
    ...
    public final void visit(Header node, FoContext data) {
        ...
        switch (node.getLevel()) {
            case 1:
                data.createFlow(node.getId(), Fo.PAGESET_CHAPTER, false, properties);
...
//                        .forEach(p -> FoDocumentRenderer.createTocEntry(p.getId(), PageUtil.encode(p.getTitle()), index.addBlock()));
    ...
    public final void visit(Image node, FoContext data) {
        ...
        FoExternalGraphic graphic = new FoExternalGraphic(node.url());
    ...
    public final void visit(Link node, FoContext data) {
        FoBasicLink link = new FoBasicLink(node.getLink())
                .setColor(Fo.COLOR);
    ...
    public final void visit(List node, FoContext data) {
        ...
        int index = 0;
        for (Node child : node) {
            FoListItem item = new FoListItem(node.isOrdered() ? String.format("%s.", index++) : "•")
                    .setSpace("0.3em");
            list.addNode(item);
    ...
    private static void createTocEntry(String id, String title, FoBlock block) {
        ...
        inline.setKeepWithNext("always")
                ...
                .addNode(leader)
                .addNode(new FoPageNumberCitation(id));

        block.setTextAlign("start")
                .setTextAlignLast("justify")
                .addNode(new FoBasicLink(id).addNode(inline));
    }
