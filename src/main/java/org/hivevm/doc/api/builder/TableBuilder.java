...
public class TableBuilder extends NodeBuilder implements Table {
    ...
    private int getColumnWidth(ColumnBuilder column) {
        if (this.columnWidth.isEmpty()) {
            ...
            int sum = this.columns.stream()
                    .collect(Collectors.summingInt(c -> ((ColumnBuilder) c).width));
     ...
     */
    private class ColumnBuilder implements Column {
    ...
    private class AreaBuilder implements Area {
     ...
     * Defines a column definition
     */
    public class CellBuilder implements Cell {

        private final RowBuilder         parent;
        private final int                rows;
        private final int                cols;
        private final CellContentBuilder content;

        private CellBuilder(RowBuilder parent, int rows, int cols) {
            this.parent = parent;
            this.rows = rows;
            this.cols = cols;
            this.content = new CellContentBuilder();
        }
        ...
        @Override
        public CellContentBuilder getContent() {
            return this.content;
        }
    }
