...
import org.hivevm.doc.api.builder.CellContentBuilder;
...
import org.hivevm.doc.api.builder.TableBuilder.RowBuilder;
...
class CodeParserApi implements CodeParser {
    ...
    private static final List<String> METHODS   = Arrays.asList("HEAD", "GET", "POST", "PUT", "DELETE");
    private static final List<String> METHOD_FG = Arrays.asList("#aaaaaa", "#61affe", "#49cc90",
            "#fca130",
            "#f13e3e");
    private static final List<String> METHOD_BG = Arrays.asList("#eeeeee", "#c7e3ff", "#bfedd8",
            "#fdcf96",
            "#fbcbcb");
    ...
    private static final Pattern PATTERN       = Pattern.compile(CodeParserApi.PATTERN_TEXT,
            Pattern.CASE_INSENSITIVE);
    private static final Pattern PATTERN2      = Pattern.compile(CodeParserApi.PATTERN_IMAGE,
            Pattern.CASE_INSENSITIVE);
     ...
     */
    @Override
    public final void generate(String text, ContainerBuilder builder, Properties properties) {
        TableBuilder table = ((DocumentBuilder) builder).addVirtualTable();
        String method = "";
        String content = null;
            ...
            if (matcher.find()) {
                ...
                if ("content".equalsIgnoreCase(name)) {
                    ...
                    if ("method".equalsIgnoreCase(name)) {
                        ...
                        CellContentBuilder cell = row.addCell(1, 2).getContent().setPadding("0pt", "3pt");
                        cell.addFenced().setPadding("15pt", "5pt")
                                .setBackground(getColor(method))
                                .setBold().getFlowBuilder().addText(method.toUpperCase());
                        cell.addFenced().setPadding("5pt", "5pt").setBold()
                                .getFlowBuilder().addText(value);
                    } else if ("response".equalsIgnoreCase(name) && (value != null)) {
                        String background = getStatusBackground(value);
                        RowBuilder row = table.addRow();
                        row.addCell(1, 2).getContent().addFenced().setPadding("5pt", "2pt")
                                .setBackground(background).setBold().getFlowBuilder().addText(name + " " + value);
                    } else {
                        RowBuilder row = table.addRow();
                        row.addCell(1, 2).getContent().addFenced().setPadding("5pt", "2pt")
                                .setBackground("#ffffff").setBold().getFlowBuilder().addText(name);
                    }
                } else if (intend > 0) {
                    ...
                    if ("content".equalsIgnoreCase(name)) {
                        ...
                        RowBuilder row = table.addRow();
                        row.addCell(1, 1).getContent()
                                .setPadding("5pt", "2pt").addFenced()
                                .setPadding("15pt", "5pt").setBold()
                                .getFlowBuilder().addText(name);
                        row.addCell(1, 1).getContent().setPadding("5pt", "2pt").addFenced()
                                .setPadding("15pt", "5pt")
                                .getFlowBuilder().addText(value);
                    }
                }
    ...
    protected final void processContent(TableBuilder table, String content, String contentType) {
        RowBuilder row = table.addRow();
        CellContentBuilder paragraph = row.addCell(1, 2).getContent()
                .setPadding("0pt", "0pt", "2pt", "0pt");
...
//            builder.setBackground("#fff");
            ...
            if (matcher.find()) {
                ...
                paragraph.getFlowBuilder().addImage(source, null, null, width, null);
            } else {
                paragraph.getFlowBuilder().addText(content);
            }
        } else {
            paragraph.addCode().getFlowBuilder().addText(content);
        }
    }
}
