...
class CodeParserApi implements CodeParser {
            ...
            if (matcher.find()) {
                ...
                if ("content".equalsIgnoreCase(name)) {
                    ...
                    if ("method".equalsIgnoreCase(name)) {
                        ...
                        RowBuilder row = table.addRow();
                        row.addCell(1, 2).getContent().addInline().setPadding("5pt", "2pt")
                                .setBackground(background).setBold()
                                .addContent(name + " " + value);
                    } else {
                        RowBuilder row = table.addRow();
                        row.addCell(1, 2).getContent().addInline().setPadding("5pt", "2pt")
                                .setBackground("#ffffff").setBold()
                                .addContent(name);
                    }
                } else if (intend > 0) {
                    ...
                    if ("content".equalsIgnoreCase(name)) {
                        ...
                        RowBuilder row = table.addRow();
                        row.addCell(1, 1).getContent().setPadding("5pt", "2pt").addInline()
    ...
    protected final void processContent(TableBuilder table, String content, String contentType) {
        RowBuilder row = table.addRow();
        ParagraphBuilder paragraph = row.addCell(1, 2).getContent()
