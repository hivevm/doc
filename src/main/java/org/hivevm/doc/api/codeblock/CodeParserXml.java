        ...
        while (matcher.find()) {
            ...
            if (matcher.group(2) != null) {
                if (matcher.start(2) > index) {
                    builder.addInline(text.substring(index, matcher.start(2))).setBold()
                            .setColor(CodeToken.TEXT.COLOR);
                }
            ...
            if (matcher.group(3) != null) {
                if (matcher.start(3) > index) {
                    builder.addInline(text.substring(index, matcher.start(3))).setBold()
                            .setColor(CodeToken.TEXT.COLOR);
                }
