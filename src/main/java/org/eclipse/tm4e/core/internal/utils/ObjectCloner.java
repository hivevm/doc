...
public final class ObjectCloner {
	...
	private static <@NonNull T> T deepClone(final T obj, final Map<Object, @Nullable Object> clones) {
		...
		if (obj instanceof final List<?> list) {
            var listClone = shallowClone(list, () -> new ArrayList<>(list));
			clones.put(list, listClone);
// TODO			listClone.replaceAll(v -> deepCloneNullable(v, clones));
			return (T) listClone;
		}
...
// TODO			mapClone.replaceAll((k, v) -> deepCloneNullable(v, clones));
			return (T) mapClone;
		}
