...
final class LineTokenizer {
	...
	private LineTokenizer(final Grammar grammar, final String lineText, final boolean isFirstLine, final int linePos,
	...
	private void scanNext() {
		LOGGER.log(TRACE, () -> "@@scanNext: |" + lineText.replace("\n", "\\n").substring(linePos) + '|');
		...
		if (matchedRuleId.equals(RuleId.END_RULE)) {
			...
			final var scopeName = rule.getName(lineText, captureIndices);
			...
			if (rule instanceof final BeginEndRule pushedRule) {
				...
				final var contentName = pushedRule.getContentName(lineText, captureIndices);
	...
	private @Nullable MatchResult matchRule(final Grammar grammar, final String lineText, final boolean isFirstLine, final int linePos,
	...
	private @Nullable MatchResult matchRuleOrInjections(final Grammar grammar, final String lineText, final boolean isFirstLine,
	...
	private @Nullable MatchInjectionsResult matchInjections(final List<Injection> injections, final Grammar grammar,
			final String lineText, final boolean isFirstLine, final int linePos, final StateStack stack, final int anchorPosition) {
	...
	private void handleCaptures(final Grammar grammar, final String lineText, final boolean isFirstLine, final StateStack stack,
			final LineTokens lineTokens, final List<@Nullable CaptureRule> captures, final OnigCaptureIndex[] captureIndices) {
		if (captures.isEmpty()) {
			return;
		}

		final var lineTextContent = lineText;
		...
		for (int i = 0; i < len; i++) {
			...
			if (retokenizeCapturedWithRuleId.notEquals(RuleId.NO_RULE)) {
				...
				final var onigSubStr = lineTextContent.substring(0, captureIndex.end());
	 ...
	 */
	private WhileCheckResult checkWhileConditions(Grammar grammar, String lineText, boolean isFirstLine, int linePos,
			StateStack stack, LineTokens lineTokens) {
	...
	static TokenizeStringResult tokenizeString(final Grammar grammar, final String lineText, final boolean isFirstLine,
